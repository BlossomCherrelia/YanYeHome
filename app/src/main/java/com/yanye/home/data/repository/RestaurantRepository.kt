...
class RestaurantRepository(
    ...
    fun observeRestaurants(): Flow<List<Restaurant>> =
        restaurantDao.observeRestaurantsForCouple(syncSettings.identity().coupleId)
    ...
    suspend fun saveRestaurant(restaurant: Restaurant): Long {
        ...
        val scopedRestaurant = restaurant.copy(
            coupleId = restaurant.coupleId ?: identity.coupleId,
            ownerUserId = restaurant.ownerUserId ?: identity.localUserId
        )
        val entity = scopedRestaurant.toEntity(
    ...
    suspend fun pendingSyncRestaurants(): List<Restaurant> =
        restaurantDao.pendingSyncRestaurantsForCouple(syncSettings.identity().coupleId)
            .map(RestaurantEntity::toDomain)

    suspend fun saveRemoteRestaurant(restaurant: Restaurant): Long {
        if (restaurant.coupleId != syncSettings.identity().coupleId) return 0L
        val now = System.currentTimeMillis()
        ...
        val identity = syncSettings.identity()
        val local = restaurant.remoteId?.let { restaurantDao.findByRemoteIdForCouple(it, identity.coupleId) }
