...
exports.main = async (event) => {
  try {
    ...
    const pendingRestaurants = Array.isArray(body.pendingRestaurants)
      ? body.pendingRestaurants
      : [];
    ...
    for (const restaurant of pendingRestaurants) {
      const remoteId = restaurant.remoteId || db.collection("restaurants").doc().id;
      const payload = {
        ...restaurant,
        remoteId,
        coupleId,
        ownerUserId: restaurant.ownerUserId || userId,
        remoteUpdatedAt: now,
    ...
    return jsonResponse(200, {
      ok: true,
      uploaded,
      restaurants: queryResult.data || []
    });
  } catch (error) {
    return jsonResponse(400, {
      ok: false,
      error: error.message || "syncRestaurants failed"
    });
  }
};
