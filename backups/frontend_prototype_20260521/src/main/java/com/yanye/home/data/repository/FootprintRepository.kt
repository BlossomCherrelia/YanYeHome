...
class FootprintRepository(
    ...
    suspend fun pendingSyncProvinceLights(): List<ProvinceLight> =
        footprintDao.pendingSyncProvinceLights().map(ProvinceLightEntity::toDomain)

    suspend fun pendingSyncCityLights(): List<CityLight> =
        footprintDao.pendingSyncCityLights().map(CityLightEntity::toDomain)

    suspend fun pendingSyncCityMemories(): List<CityMemory> =
        footprintDao.pendingSyncCityMemories().map(CityMemoryEntity::toDomain)
    ...
    suspend fun saveRemoteProvinceLight(provinceLight: ProvinceLight): Long {
        ...
        val local = provinceLight.remoteId?.let { footprintDao.findProvinceLightByRemoteId(it) }
            ?: footprintDao.findProvinceLightByName(provinceLight.provinceName)
    ...
    suspend fun saveRemoteCityLight(cityLight: CityLight): Long {
        ...
        val local = cityLight.remoteId?.let { footprintDao.findCityLightByRemoteId(it) }
            ?: footprintDao.findCityLightByName(cityLight.provinceName, cityLight.cityName)
    ...
    suspend fun saveRemoteCityMemory(cityMemory: CityMemory): Long {
        ...
        val local = cityMemory.remoteId?.let { footprintDao.findCityMemoryByRemoteId(it) }
            ?: footprintDao.findCityMemoryByCity(cityMemory.provinceName, cityMemory.cityName)
